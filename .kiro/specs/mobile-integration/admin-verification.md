...
## WebView 운영 후속 점검 — 2026-09-24 KST

- iOS 업데이트 주소를 포퐁의 실제 앱 ID `6814126823`으로 제한함. 다른 앱 ID, 같은 숫자로 시작하는 별도 ID, ID 뒤의 임의 경로를 거절함. 국가/앱 이름 경로·언어 쿼리·끝 슬래시는 허용함.
- 새 버전 정책 폼에 포퐁 iOS/Android 상세 주소를 기본으로 채우고, 스토어에서 설치할 수 있는 버전을 기준으로 정책을 설정하도록 안내함. 현재 스토어 공개 여부를 자동 승인하는 기능은 아님.
- 잘못된 기존 정책도 끌 수 있도록 기존 목록 스위치는 `{ isActive: false }`만 전송함. 백엔드의 비활성화 검증 예외와 결합되는 경로이며 운영 정책을 실제 변경하지 않음.
- Node 테스트 37개, 변경 파일 ESLint, 타입 검사, production build, diff 공백 검사 통과함. Vite의 기존 500KB 청크 크기 안내는 남아 있음.
- API 계약 검사: 호출 141개 불일치 0개, 기능 커버리지 129/135임. 기존 약관 관리 GET/POST/GET detail/PATCH/DELETE/activate 6개 미구현으로 명령은 실패함. 이번 앱 버전 수정으로 해당 기존 범위를 확장하지 않음.
- 위 이전 절의 `main` 미푸시는 당시 개별 검증 작업 기준임. 후속 변경은 사용자의 지시에 따라 `dev`와 `main`을 같은 커밋으로 동기화함.
