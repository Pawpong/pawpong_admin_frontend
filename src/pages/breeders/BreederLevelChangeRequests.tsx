...
import { Table, Tag, Button, Modal, Form, Input, message, Space, Descriptions, Image, Card, Checkbox } from 'antd';
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="small" wrap>
          ...
          <Button danger icon={<CloseCircleOutlined />} onClick={() => handleRejectClick(record)} size="small">
            거절
          </Button>
        </Space>
      ),
            ...
            <div className="space-y-4">
              {selectedBreeder.verificationInfo.documents?.map((doc, index) => (
                <div key={index} className="border rounded p-3">
                  ...
                  {doc.fileUrl && (
                    <Image src={doc.fileUrl} alt={doc.type} style={{ maxWidth: '100%', maxHeight: '400px' }} />
                  )}
                ...
                <Checkbox
                  ...
                  onChange={(e) => {
                    if (e.target.checked) {
                      ...
                      form.setFieldsValue({
                        rejectionReason: currentReason ? `${currentReason}\n- ${reason}` : `- ${reason}`,
                      });
                    }
                  }}
                >
