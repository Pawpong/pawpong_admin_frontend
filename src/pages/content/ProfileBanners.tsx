...
import { profileBannerApi, type ProfileBanner, type ProfileBannerRequest } from '../../features/user/api/userApi';
...
const ProfileBanners = () => {
    ...
    const fetchBanners = async () => {
        try {
            setLoading(true);
            const data = await profileBannerApi.getAllBanners();
            // 순서대로 정렬
            const sortedData = data.sort((a, b) => a.order - b.order);
            setBanners(sortedData);
        } catch (error) {
            message.error('프로필 배너 목록 조회 실패');
            console.error(error);
        } finally {
            setLoading(false);
        }
                ...
                return (
                    <span
                        style={{
                            ...
                            borderRadius: '4px',
                            backgroundColor:
                                bannerType === 'login' ? 'var(--color-primary-100)' : 'var(--color-secondary-100)',
        ...
        },
        {
            ...
            render: (isActive: boolean, record: ProfileBanner) => (
                <Switch
                    ...
                    onChange={() => handleToggleActive(record.bannerId, isActive !== false)}
                />
            ),
        },
        {
            ...
            render: (_: unknown, record: ProfileBanner) => (
                <Space size="middle" onClick={(e) => e.stopPropagation()}>
                    ...
                    <Popconfirm
                        title="프로필 배너 삭제"
                        description="정말 이 프로필 배너를 삭제하시겠습니까?"
