    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      >
        ...
        <div style={{ marginBottom: 16, display: 'flex', alignItems: 'center', gap: 16 }}>
          <Button icon={<ArrowLeftOutlined />} disabled={currentPath.length === 0} onClick={handleGoBack}>
            뒤로
          </Button>
          <Breadcrumb
            items={[
              ...
              ...currentPath.map((folder, index) => ({
                title: (
                  <span style={{ cursor: 'pointer' }} onClick={() => handleBreadcrumbClick(index)}>
                    {folder}
                  </span>
                ),
              })),
        ...
        {currentPath.length > 0 && (
          <div style={{ marginBottom: 16, padding: '8px 16px', background: '#fafafa', borderRadius: 8 }}>
            <Space size="large">
              <span>
                현재 폴더: <strong>{currentStats.total}</strong>개 파일
              </span>
              <span>
                용량: <strong>{formatFileSize(currentStats.totalSize)}</strong>
              </span>
              ...
              if (item.type === 'folder') {
                ...
                return (
                  ...
                  >
                    ...
                    {/* 참조 상태 태그 */}
                    <Tag color={item.isReferenced ? 'success' : 'warning'} style={{ fontSize: 10 }}>
