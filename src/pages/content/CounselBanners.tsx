...
import { counselBannerApi, type CounselBanner, type CounselBannerRequest } from '../../features/user/api/userApi';
...
const CounselBanners = () => {
    ...
    const fetchBanners = async () => {
        try {
            setLoading(true);
            const data = await counselBannerApi.getAllBanners();
            // 순서대로 정렬
            const sortedData = data.sort((a, b) => a.order - b.order);
            setBanners(sortedData);
        } catch (error) {
            message.error('상담 배너 목록 조회 실패');
            console.error(error);
        } finally {
            setLoading(false);
        }
