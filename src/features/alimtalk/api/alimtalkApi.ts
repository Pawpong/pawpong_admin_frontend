...
export const alimtalkApi = {
  ...
  getTemplates: async (): Promise<AlimtalkTemplate[]> => {
    const response = await apiClient.get<ApiResponse<AlimtalkTemplateListResponse>>(
      '/alimtalk-admin/templates'
    );
  ...
  getTemplateByCode: async (templateCode: string): Promise<AlimtalkTemplate> => {
    const response = await apiClient.get<ApiResponse<AlimtalkTemplate>>(
  ...
  createTemplate: async (createData: AlimtalkTemplateCreateRequest): Promise<AlimtalkTemplate> => {
    const response = await apiClient.post<ApiResponse<AlimtalkTemplate>>(
      '/alimtalk-admin/templates',
      createData
    );
    return response.data.data;
