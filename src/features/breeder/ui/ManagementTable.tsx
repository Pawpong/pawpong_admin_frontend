...
import { StopOutlined, EyeOutlined, CheckCircleOutlined, ExperimentOutlined } from '@ant-design/icons';
...
export function ManagementTable({
  ...
  const columns: ColumnsType<BreederVerification> = [
    ...
    { title: '전화번호', dataIndex: 'phoneNumber', key: 'phoneNumber', width: 130, render: (p: string) => p || '-' },
    {
      ...
      key: 'approvedAt',
      width: 150,
      render: (d: string) => (d ? new Date(d).toLocaleDateString('ko-KR') : '-'),
    },
    {
      title: '계정 상태',
      ...
      key: 'accountStatus',
      width: 100,
      render: (s: string) => (s === 'suspended' ? <Tag color="red">정지됨</Tag> : <Tag color="green">활성</Tag>),
    },
    {
      title: (
      ...
      render: (v: boolean, r) => (
        <Switch
          checked={v || false}
          onChange={(c) => onTestAccountToggle(r, c)}
          checkedChildren="ON"
    ...
    },
    {
      ...
      render: (_, r) => {
        ...
        return (
          <Space size="small">
            <Tooltip title="상세 보기">
              <Button type="link" icon={<EyeOutlined />} onClick={() => onViewDetails(r)}>
                상세
              </Button>
            </Tooltip>
            {isSuspended ? (
              <Tooltip title="정지 해제">
                <Button icon={<CheckCircleOutlined />} onClick={() => onUnsuspend(r)} size="small">
                  정지 해제
                </Button>
              </Tooltip>
            ) : (
              <Tooltip title="계정 정지">
                <Button danger icon={<StopOutlined />} onClick={() => onSuspend(r)} size="small">
                  정지
                </Button>
              </Tooltip>
            )}
