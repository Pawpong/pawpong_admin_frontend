...
export const platformApi = {
    ...
    getPhoneWhitelist: async (): Promise<PhoneWhitelistListResponse> => {
        const response = await apiClient.get<ApiResponse<PhoneWhitelistListResponse>>(
            '/platform-admin/phone-whitelist',
        );
        return response.data.data;
    },
    ...
    updatePhoneWhitelist: async (id: string, data: PhoneWhitelistUpdateRequest): Promise<PhoneWhitelist> => {
        const response = await apiClient.patch<ApiResponse<PhoneWhitelist>>(
            `/platform-admin/phone-whitelist/${id}`,
            data,
        );
        return response.data.data;
    },
